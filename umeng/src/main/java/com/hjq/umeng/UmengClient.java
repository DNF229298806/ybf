...
public final class UmengClient {
    ...
    public static void share(Activity activity, Platform platform, UmengShare.ShareData data, UmengShare.OnShareListener listener) {
        if (UMShareAPI.get(activity).isInstall(activity, platform.getThirdParty())) {
            ...
            // 当分享的平台软件可能没有被安装的时候
            if (listener != null) {
                listener.onError(platform, new PackageManager.NameNotFoundException(platform.getAppName() + "未安装"));
            }
        }
    }
    ...
    public static void login(Activity activity, Platform platform, UmengLogin.OnLoginListener listener) {
        if (UMShareAPI.get(activity).isInstall(activity, platform.getThirdParty())) {
            try {
                ...
                Log.d("um", ignored.getMessage());
            }

        } else {
            // 当登录的平台软件可能没有被安装的时候
            if (listener != null) {
                listener.onNotInstall(platform.getAppName() + "未安装");
            }
        }
    }
